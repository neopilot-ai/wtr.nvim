...
local wtr_path = function(prompt_bufnr)
    local selection = action_state.get_selected_entry(prompt_bufnr)
    return selection.path
end

local switch_worktree = function(prompt_bufnr)
    local worktree_path = wtr_path(prompt_bufnr)
    actions.close(prompt_bufnr)
    if worktree_path ~= nil then
        wtr.switch_worktree(worktree_path)
    end
end
...
local confirm_deletion = function(forcing)
    if not wtr._config.confirm_telescope_deletions then
        return true
    end
...
local delete_worktree = function(prompt_bufnr)
    ...
    actions.close(prompt_bufnr)
    if worktree_path ~= nil then
       wtr.delete_worktree(worktree_path, force_next_deletion, {
        ...
        actions.select_default:replace(
            function(prompt_bufnr, _)
                ...
                create_input_prompt(function(name)
                    ...
                    wtr.create_worktree(name, branch)
                end)
            end)
...
local telescope_wtr = function(opts)
    opts = opts or {}
